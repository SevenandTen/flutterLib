...
// ignore: non_constant_identifier_names
  ...
  static void showTimePicker({
    ...
    assert(beginYear < endYear);
      showModalBottomSheet(context: context, backgroundColor:  Colors.transparent,builder: (context) {
        return WYTimeSelectView(timeType: timeType,beginYear: beginYear,endYear: endYear,hasLongTime: hasLongTime,currentDate:nowDate,callBack: callBack,);
      });
  ...
  static void showTimeSelector({
    ...
    Navigator.push(context, CupertinoPageRoute(builder: (context){
      return WYDateSelectView(callBack: callBack,type: supportAnyDay == true ? 2 : 1,);
    }) );
  }
...
class WYTimeSelectView extends StatefulWidget {
  ...
  const WYTimeSelectView({Key? key, this.callBack, this.timeType, this.hasLongTime, this.beginYear, this.endYear, this.currentDate}) : super(key: key);
  @override
  State<StatefulWidget> createState() {
    return WYTimeSelectState();
  }

}

class WYTimeSelectState extends State<WYTimeSelectView> {
      ...
      child:  Column(children: [
        ...
        if(widget.hasLongTime == true)
         GestureDetector(
           ...
           child:  Container(height: 50,color: Colors.transparent,child: Row(crossAxisAlignment: CrossAxisAlignment.center,children: [
             Spacer(),
...
class WYDateSelectView extends StatefulWidget {
  final Function(int startTime , int endTime)? callBack;
  final int type ; // 1为不跨月 2 为跨月
  final int beginYear;

  const WYDateSelectView({Key? key, this.callBack, this.type = 1, this.beginYear = 2015,}) : super(key: key);
...
class WYDateSelectState extends State<WYDateSelectView> {
...
class WYCalendarSelectState extends State<WYDateSelectView> {
